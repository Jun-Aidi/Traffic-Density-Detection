  ...
  return (
    <>
      {/* --------------------
          DESKTOP SIDEBAR
          -------------------- */}
      <aside className="hidden md:flex fixed top-0 left-0 h-screen w-64 bg-white border-r border-slate-200 flex-col z-40 shadow-sm">
        {/* Logo */}
      ...
      {/* --------------------
          MOBILE BOTTOM NAV
          -------------------- */}
      <nav className="md:hidden fixed bottom-0 left-0 w-full h-16 bg-white border-t border-slate-200 z-50 flex items-center justify-between px-2 shadow-[0_-4px_10px_rgba(0,0,0,0.05)] pb-safe">
          ...
          {leftItems.map((item) => (
            <button
              ...
              className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${
                activePage === item.id ? 'text-brand-600' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
              {item.icon}
        ...
        <div className="relative w-16 flex justify-center h-full">
          <div className="absolute -top-6">
            <button 
              ...
              className="w-14 h-14 rounded-full bg-gradient-to-br from-brand-400 to-brand-600 flex items-center justify-center shadow-lg shadow-brand-500/30 border-4 border-slate-100 text-white transform transition-transform active:scale-95"
            >
